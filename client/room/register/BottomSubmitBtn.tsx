...
function BottomSubmitBtn() {
  ...
  const checkHandler = async () => {
    ...
    const utility_arr = ['전기세', '가스비', '수도세', '인터넷', 'TV'];
    const option_arr = ['에어컨', '냉장고', '세탁기', '가스레인지', '인덕션', '전자레인지', '책상', '책장', '침대', '옷장', '싱크대', '신발장',];
    const advantage_arr = ['엘리베이터','주차공간','반려동물','여성전용','전세대출'];
    ...
    let utilityPush:string[] = [];
    for (let i = 0; i < utility_arr.length; i++) {
      if (utility[i] === 1) {
        utilityPush.push(utility_arr[i]);
      }
    };
    formData.append('utility', utilityPush.toString());
    ...
    let optionPush:string[] = [];
    for (let i = 0; i < option_arr.length; i++) {
      if(option[i] === 1) {
        optionPush.push(option_arr[i]);
      }
    };
    formData.append('option', optionPush.toString());
    //advantage 전송 ("엘리베이터, 주차공간, 반려동물, 여성전용, 전세대출")
    let advantagePush:string[] = [];
    for (let i = 0; i < advantage_arr.length; i++) {
      if(advantage_stateArr[i] === 1) {
        advantagePush.push(advantage_arr[i])
      } 
    };
    formData.append('advantage', advantagePush.toString());
    //tenancyAgreement 전송
    if(tenancyAgreement === 0) {
      formData.append('tenancyAgreement', String(false))
    } else if(tenancyAgreement === 1) {
      formData.append('tenancyAgreement', String(true))
    }
